...
class BeritaAcaraFormPage extends StatefulWidget {
  ...
  @override
  State<BeritaAcaraFormPage> createState() => _BeritaAcaraFormPageState();
}
...
class _BeritaAcaraFormPageState extends State<BeritaAcaraFormPage> {
  ...
  void _clearForm() {
    ...
    setState(() {
      ...
      _filePath  = null;
      _fileName  = null;
      _assignFinance = false;
      _assignSales   = false;
    });
  }

  Future<void> _submit() async {
    if (!_assignFinance && !_assignSales) {
      _snack('Pilih minimal 1 department tujuan!');
      return;
    }
    ...
    // Cek apakah BRD sudah di-approve semua pihak
    final brdApproved = await vm.isBrdFullyApproved();
    if (!brdApproved) {
      _snack('BRD belum disetujui semua pihak! Tunggu approval Finance & Engineering dulu.');
      return;
    }

    final parts = <String>[];
    if (_assignFinance) parts.add('Finance Department');
    if (_assignSales)   parts.add('Sales Department');
    ...
    if (ok && mounted) {
      _snack('Berita Acara berhasil dikirim! ✅');
      _clearForm();
    }
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: const Padding(
          ...
          child: Icon(Icons.article, size: 22),
        ),
        title: const Text(
          'Technical Report / Berita Acara',
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Banner peringatan kalau BRD belum di-approve
            FutureBuilder<bool>(
              future: context.read<DocumentViewModel>().isBrdFullyApproved(),
              builder: (ctx, snap) {
                if (snap.data == false) {
                  return Container(
                    ...
                    child: const Row(children: [
                      ...
                      Expanded(
                        child: Text(
                          'Tunggu BRD disetujui oleh Finance & Engineering terlebih dahulu.',
            ...
            Wrap(
              ...
              children: [
                _CheckItem(
                  label: 'Finance',
                  value: _assignFinance,
                  onChanged: (v) => setState(() => _assignFinance = v!),
                ),
                _CheckItem(
                  label: 'Finance & Sales',
                  value: _assignFinance && _assignSales,
            ...
            // TASK TITLE
            _label('Task Title'),
            _field(_titleCtrl, 'Contoh: Setup ICU Monitoring System B'),
            const SizedBox(height: 12),
            ...
            TextField(
              ...
              maxLines: 5,
              decoration: _dec('Deskripsikan hasil pengerjaan...'),
            ),
            const SizedBox(height: 12),
...
// Widget checkbox kecil untuk pilihan assign to
